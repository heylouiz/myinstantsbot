    ...
    for instant in results:
        inline_results.append(
            InlineQueryResultVoice(
                id=uuid4(), title=instant["text"], voice_url=instant["url"]
            )
        )
...
def get_voice(update, context):
    ...
    if update.message.voice.file_size > 307200:  # 300kb
        update.message.reply_text(
            "Error: File must be smaller than 300kb, has {}\n\nAborting...".format(
    ...
    update.message.reply_text(
        "Nice, everything is fine with your audio!\n"
...
def get_audio(update, context):
    ...
    if update.message.audio.file_size > 307200:  # 300kb
        update.message.reply_text(
            "Error: File must be smaller than 300kb, has {}\n\nAborting...".format(
                update.message.audio.file_size / 1024
            )
        )
        return ConversationHandler.END
...
def get_name(update, context):
    ...
    update.message.reply_text(
        'Your instant name will be "{}".\nAre you sure about it? Send Yes, No or /cancel'.format(
...
def name_confirmation_and_upload(update, context):
    ...
    if update.message.text not in ["Yes", "No"]:
        update.message.reply_text(
            "Invalid confirmation word!\nTry again or send /cancel"
        )
        return CONFIRMATION

    if update.message.text == "No":
        update.message.reply_text(
            "Ok, let's pick another name.\nSend me a new name or send /cancel to abort."
        )
        return NAME

    if update.message.text == "Yes":
        update.message.reply_text(
            'All right, your audio will be uploaded with the name "{}".'.format(
                context.user_data["name"]
            )
        )
    ...
    update.message.reply_text(
        "Instant was successfully sent, you should be able to search for it in a while.\n"
...
def main():
    ...
    for var in ["MYINSTANTS_USERNAME", "MYINSTANTS_PASSWORD", "TELEGRAM_TOKEN"]:
        if var not in os.environ:
            LOGGER.error(
                f"Missing environment variable {var}! See README.md file for more information."
            )
            return 1
